...
(function () {
  ...
  var self = function (opts) {
    ...
    this.multicast = opts.multicast || true
  ...
  self.prototype.createSocket = function (address, port) {
    ...
    socket.on('error', function(err) {
      that.emit('error', {type: 'socket', socket: socket, error: err})
    })

    socket.on('message', function(message, referrer) {
      that.emit('message', {message: message, referrer: referrer})
    ...
    socket.on('listening', function() {
      try {
        ...
        that.emit('error', {type: 'listenSocket', socket: socket})
        return
      }
      ...
      var addMember = function () {
        try {
          ...
          if (e.code === 'ENODEV' || e.code === 'EADDRNOAVAIL') {
            that.emit('delay', {type: 'socketMembership', address: addr.address, port: addr.port})
            setTimeout(function() {
              addMember()
            }, 5000)
          } else {
            that.emit('error', {type: 'socketMembership', address: addr, error: e})
          }
        }
      }
      addMember()
    ...
    socket.bind({port: port, address: address}, function() {
  ...
  self.prototype.parseCommand = function (msg, rinfo) {
    ...
    switch (method) {
      ...
      case constants.msearch:
        ...
        break
      default:
        this.emit('error', {type: 'command', subType: 'unhandled', message: msg, rinfo: rinfo})
    }
  }
  ...
  self.prototype.notify = function (headers, msg, rinfo) {
    ...
    switch (headers.NTS.toLowerCase()) {
      case constants.alive:
        this.emit(constants.advertiseAlive, {headers: headers, rinfo: rinfo})
        break
      case constants.bye:
        this.emit(constants.advertiseBye, {headers: headers, rinfo: rinfo})
        break
      default:
        this.emit('error', {type: 'notify', subType: 'unhandled', message: msg, rinfo: rinfo})
    }
  }
  ...
  self.prototype.send = function (message, host, port) {
    ...
    for (var i = 0; i < this.sockets.length; i++) {
      var socket = this.sockets[i]
      socket.send(message, 0, message.length, targetPort, targetHost, function(err) {
        if (err) {
          self.emit('error', {type: 'send', error: err})
        } else {
